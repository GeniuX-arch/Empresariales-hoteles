...
const CrearUsuario = () => {
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg w-full max-w-md p-6 mx-auto">
      <h2 className="text-center text-2xl font-bold mb-6">{id ? 'Editar Usuario' : 'Crear Usuario'}</h2>
      <form onSubmit={handleSubmit}>
        ...
        <label htmlFor="contrasena" className="block font-bold text-gray-700 mb-1">
          Contraseña:
        </label>
        <input
          type="password"
        ...
        <label htmlFor="tipoUsuario" className="block font-bold text-gray-700 mb-1">
          Tipo de Usuario:
        </label>
        <select
          id="tipoUsuario"
        ...
        >
          {isLoading ? 'Guardando...' : (id ? 'Actualizar Usuario' : 'Guardar Usuario')}
        </button>
      </form>
    </div>
  );
